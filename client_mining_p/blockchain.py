...
def mine():
    ...
    if not all(k in data for k in required):
        return jsonify({error: "The required data is not present"}), 400
    ...
    if blockchain.valid_proof(last_block_string, new_proof) is False:
        return jsonify({'error': 'Proof Invalid'}), 400
    else:
